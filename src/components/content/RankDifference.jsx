...
const RankDifference = () => {
    ...
    // const rankDifferences=0
    const rank = rankIndicator.countries?.map((country, index) => (<span key={index}>{country.rank_difference}</span>))

    const country = rankIndicator.countries?.map((country, index) => (<span key={index}>{country.country}</span>))

    return (
        <section className='w-2/6 mx-auto mt-8 text-white  rounded-md'>

            <div className='h-[56px] bg-[#0D1F3D]'>
                <p className='text-[#A7B4CA]'>Difference in rank between years</p>
            </div>
            ...
            <div className='bg-[#051124] p-2'>
                ...
                <div className='h-[331px] flex justify-between mt-6'>
                    <div className='flex'>
                        ...
                        <span>
                            {
                                ...
                                // flag.filter((flag) => flag.name.common === data.country).map((flag) => ())

                                //add flags from restcountries api , no comments

                                    flag.filter((flag) => flag.name.common === country).map((flag) => (
